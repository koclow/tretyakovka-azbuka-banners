...
// node render.js <file.html> <seconds> <out.mov> [fps=50] [alpha=0]
const puppeteer = require('puppeteer-core');
const { spawn } = require('child_process');
const path = require('path');
const [,, html, secArg, out, fpsArg = '50', alphaArg = '0'] = process.argv;
const sec = +secArg, fps = +fpsArg, alpha = alphaArg === '1';
const W = 1920, H = 1080, N = Math.round(sec * fps);
(async () => {
  const browser = await puppeteer.launch({
    executablePath: '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',
    headless: true, args: ['--hide-scrollbars', '--force-device-scale-factor=1', '--disable-gpu-vsync', '--font-render-hinting=none']
  });
  const page = await browser.newPage();
  await page.setViewport({ width: W, height: H, deviceScaleFactor: 1 });
  await page.goto('file://' + path.resolve(html) + '?render', { waitUntil: 'load' });
