...
MATTE = np.array([131, 26, 30], np.uint8)   # цвет красных полей РБК; в их кадре под полями просвечивал старый баннер
...
while True:
    ...
    if F0 <= f < F1 + EX:
        ...
        if B is not None:
            used += 1
            # прямоугольник программы: линейно от полного кадра к окну и обратно
            if f < F0 + SQ: k = (f - F0) / SQ
            elif f >= F1: k = 1 - (f - F1) / EX
            else: k = 1.0
            left = round(k * WIN_LEFT); bottom = round(H - k * (H - WIN_BOTTOM))
            outf = T.copy()
